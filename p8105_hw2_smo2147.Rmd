...
```{r checking for NA neighborhoods}
zillow_joined_df |> 
...
```{r checking for duplicates per zipcode}
zillow_joined_df |> 
...
The final **`zillow_joined_df`** dataset contains **`r nrow(zillow_joined_df)`** observations and **`r ncol(zillow_joined_df)`**. The dataset also contains **`r zillow_joined_df |> distinct(zip_code) |> nrow()`** unique zipcodes and **`r zillow_joined_df |> distinct(neighborhood) |> nrow()`** unique neighborhoods. 

###### Zip codes Differences by Dataset

```{r zip codes differences by dataset}
...
One reason zip codes might be missing in the **`zillow_zori_df`** dataset but not in the **`zillow_codes_df`** dataset is because some might be more recent listing, or Zillow simply does not update them frequently. 

###### Rental Prices Comparison 2021 vs 2020

```{r setting variables for rental price comparison}
...
In the table above we see that the top 10 zip codes with the largest rental price drops between January of 2020 and 2021. We can see that these zip codes are located in Manhattan. Price drops might be due to people moving out of NYC during the pandemic, which made prices go down to incentize people to move into those zip codes.
